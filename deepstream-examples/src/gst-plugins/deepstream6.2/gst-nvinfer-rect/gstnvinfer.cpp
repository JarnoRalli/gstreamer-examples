...
G_DEFINE_TYPE(_GstNvInfer, gst_nvinfer, GST_TYPE_BASE_TRANSFORM);
                         ...
                         GValue *value, GParamSpec *pspec)
{
  ...
  case PROP_PROCESS_MODE:
    g_value_set_enum(value,
                     nvinfer->process_full_frame ? PROCESS_MODEL_FULL_FRAME : PROCESS_MODEL_OBJECTS);
    break;
  case PROP_CONFIG_FILE_PATH:
...
gst_nvinfer_start(GstBaseTransform *btrans)
{
  ...
  auto nvtx_deleter = [](nvtxDomainHandle_t d)
  { nvtxDomainDestroy(d); };
  ...
  default:
    GST_ELEMENT_ERROR(nvinfer, LIBRARY, SETTINGS,
                      ("Unsupported network input format: %d",
                       init_params->networkInputFormat),
                      (nullptr));
    return FALSE;
  }
                    ...
                    GstNvInferObjectHistory *history)
{
  ...
  if (history && IS_CLASSIFIER_INSTANCE(nvinfer))
  {
    ...
    if ((history->last_inferred_coords.width *
         history->last_inferred_coords.height * (1 + REINFER_AREA_THRESHOLD)) <
                            ...
                            NvBufSurface *in_surf)
{
       ...
       l_frame = l_frame->next)
  {
         ...
         l_obj = l_obj->next)
    {
      ...
      frame.input_surf_params =
          (nvinfer->classifier_async_mode) ? nullptr : (in_surf->surfaceList + frame_meta->batch_id);
      batch->frames.push_back(frame);
  ...
  if (nvinfer->current_batch_num -
          nvinfer->last_map_cleanup_frame_num >
      MAP_CLEANUP_INTERVAL)
  {
...
gst_nvinfer_tensoroutput_free(GstMiniObject *obj)
{
  ...
  output_obj->infer_context->releaseBatchOutput(output_obj->batch_output);
