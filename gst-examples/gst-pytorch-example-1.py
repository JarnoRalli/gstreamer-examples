...
def on_frame_probe(pad_in, info_in, detector_in, transform_in, device_in, detection_threshold_in):
    """
    ...
    with nvtx_range("on_frame_probe"):
        ...
        with nvtx_range("inference"):
            with torch.no_grad():
                detections = detector_in(image_tensor)

        with nvtx_range("postprocessing"):
            results_per_input = ssd_utils.decode_results(detections)
            best_results_per_input = [ssd_utils.pick_best(results, detection_threshold_in) for results in
                                      results_per_input]
...
if __name__ == "__main__":
    ...
    argParser.add_argument("-d", "--detection_threshold", help="detection threshold", default=0.4, type=float)
    ...
    pipeline.get_by_name('fake_sink').get_static_pad('sink').add_probe(Gst.PadProbeType.BUFFER,
                                                                       partial(on_frame_probe,
